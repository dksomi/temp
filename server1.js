...
io.sockets.on("connection", function(socket) {
  ...
  socket.on("whoWin", function(data) {
    ...
    if(arrRpsType.length == 2) {
      io.sockets.to(arrRpsType[0].id).emit("isWin", {optType: arrRpsType[1].rpsType, isWin: isWin(arrRpsType[0].rpsType, arrRpsType[1].rpsType)});
      io.sockets.to(arrRpsType[1].id).emit("isWin", {optType: arrRpsType[0].rpsType, isWin: isWin(arrRpsType[1].rpsType, arrRpsType[0].rpsType)});
    }
  });
...
function makeUser(socket, data) {
  if(arrUsers.length ==  0) {
    ...
    arrUsers.push({id: socket.id, userId: data, opponent: arrUsers[0].userId});
  }
}
